...
def filter_tools_by_category(df, category):
    ...
    if category in df.columns:
        # Filter rows where the category column is not empty
        return df[df[category].notna() & (df[category] != '')]
    else:
        print(f"Warning: Category '{category}' not found in DataFrame columns")
        return pd.DataFrame()
...
def get_all_categories_for_tools(df, categories):
    ...
    for _, row in df.iterrows():
        product_name = None
        for col in ['PRODUCT/FEATURE\nNAME', 'PRODUCT/FEATURE NAME', 'PRODUCT NAME', 'Product Name']:
        ...
        # Find all categories this tool belongs to
        tool_cats = []
...
def generate_search_prompt(category, tools_list, category_description, all_categories, iteration=1):
...
def search_new_tools(category, all_categories, iteration=1):
    ...
    # Generate the prompt
    prompt = generate_search_prompt(category, tools_list, category_desc, all_categories, iteration)
    ...
    try:
        ...
        # Extract the JSON part and the "More tools available" indicator
        response_text = response.text
        
        # Split the response to separate JSON and "More tools available" indicator
        parts = response_text.split("More tools available:")
        
        json_text = parts[0].strip()
        # Clean up the JSON text (remove markdown code block markers if present)
        json_text = json_text.replace("```json", "").replace("```", "").strip()
        
        more_available = False
        if len(parts) > 1:
            more_available_text = parts[1].strip().lower()
            more_available = more_available_text == "true"
...
def update_new_tools_csv(tools):
    ...
    active_tools_df['unique_id'] = (active_tools_df['COMPANY'].fillna('').astype(str).str.lower().str.replace(r'\s+', '', regex=True) + 
                                    '_' + 
                                    active_tools_df['PRODUCT/FEATURE\nNAME'].fillna('').astype(str).str.lower().str.replace(r'\s+', '', regex=True))
...
def main():
    ...
    for category in categories:
        ...
        # Initialize iteration counter and more_available flag
        iteration = 1
        more_available = True
        total_tools_added = 0
        
        # Continue searching until no more tools are available or max iterations reached
        while more_available and iteration <= 5:  # Limit to 5 iterations to avoid excessive API calls
            # Search for new tools
            json_text, more_available = search_new_tools(category, categories, iteration)
            ...
            if json_text:
                ...
                if tools:
                    ...
                    # If no new tools were added, no need to continue
                    if tools_added == 0:
                        more_available = False
                else:
        ...
        print(f"Completed search for {category} category. Total tools added: {total_tools_added}")
